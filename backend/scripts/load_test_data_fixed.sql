...
BEGIN
    INSERT INTO Products (name, description, model, brand, categoryId, type, currentStock, minimumStock, active)
...
BEGIN
    INSERT INTO AssetDetails (productId, serialNumber, assetTag, purchaseDate, warrantyExpiration, status, notes)
    VALUES (@dellNotebookId, 'DL5420-001', 'DELL-001', DATEADD(month, -6, GETDATE()), DATEADD(year, 3, DATEADD(month, -6, GETDATE())), 'available', 'Equipo nuevo');
END

IF NOT EXISTS (SELECT * FROM AssetDetails WHERE serialNumber = 'DL5420-002')
BEGIN
    INSERT INTO AssetDetails (productId, serialNumber, assetTag, purchaseDate, warrantyExpiration, status, notes)
    VALUES (@dellNotebookId, 'DL5420-002', 'DELL-002', DATEADD(month, -6, GETDATE()), DATEADD(year, 3, DATEADD(month, -6, GETDATE())), 'available', 'Equipo nuevo');
END
...
BEGIN
    INSERT INTO AssetDetails (productId, serialNumber, assetTag, purchaseDate, warrantyExpiration, status, notes)
    VALUES (@samsungId, 'SG22-001', 'SAM-001', DATEADD(month, -3, GETDATE()), DATEADD(year, 2, DATEADD(month, -3, GETDATE())), 'available', 'Equipo nuevo');
END
